...
void SendDeviceItem::doRender(wxDC& dc)
{
    ...
    //device state
    if (state_printable <= 2) {
        dc.SetTextForeground(wxColour(148, 209, 176));
    }
    else {
    ...
    if (m_hover) {
        dc.SetPen(wxPen(wxColour(148, 209, 176)));
...
wxPanel* SendMultiMachinePage::create_page()
{
    ...
    // add send button
    btn_bg_enable = StateColor(std::pair<wxColour, int>(wxColour(129, 170, 147), StateColor::Pressed), std::pair<wxColour, int>(wxColour(181, 255, 214), StateColor::Hovered),
        std::pair<wxColour, int>(wxColour(148, 209, 176), StateColor::Normal));
