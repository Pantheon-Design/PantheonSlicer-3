...
SavePresetDialog::Item::Item(Preset::Type type, const std::string &suffix, wxBoxSizer *sizer, SavePresetDialog *parent) : m_type(type), m_parent(parent)
{
    ...
    // Add checkbox for "Detach from parent"
    m_detach_checkbox = new wxCheckBox(parent, wxID_ANY, _L("Detach(remove inheritance)"));
    ...
    m_detach_checkbox->Bind(wxEVT_CHECKBOX, [this](wxCommandEvent&) { 
        m_detach = m_detach_checkbox->GetValue(); 
    });
...
void SavePresetDialog::build(std::vector<Preset::Type> types, std::string suffix)
{
    ...
    StateColor btn_bg_green(std::pair<wxColour, int>(wxColour(129, 170, 147), StateColor::Pressed),
    ...
    m_confirm->SetBorderColor(wxColour(148, 209, 176));
